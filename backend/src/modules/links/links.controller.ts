...
import { createNewLink, getLink, getLinks, updateExistingLink, removeLink, resolveShortCode, getTopLinks } from "./links.service";
...
export async function createLinkHandler(c: Context) {
  try {
    ...
    const link = await createNewLink(user.id, { originalUrl: parsed.originalUrl, shortCode, expiresAt });
...
export async function topLinksHandler(c: Context) {
  try {
    ...
    return c.json(links.map(link => ({ ...link, shortUrl: `${process.env.BASE_URL}/r/${link.shortCode}` })));
  } catch (err: any) {
